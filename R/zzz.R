.onLoad <- function(libname, pkgname) {
  ...
  if (requireNamespace("dplyr", quietly = TRUE)) {
    ...
    registerS3method("collect", "ShennongData::ShennongData", .sn_collect_shennong_data, envir = ns)
    registerS3method("collect", "shennong_result", collect.shennong_result, envir = ns)
  }
  registerS3method("collect", "ShennongData::ShennongData", .sn_collect_shennong_data, envir = asNamespace(pkgname))
